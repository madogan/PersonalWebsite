...
- [x] 5. Resume section edit page — `app/admin/resume/page.tsx`; form for experience, education, coreSkills; dynamic add/remove.
...
*Completed; plan file removed from `.cursor/plans/`.*
...
## Plan: Admin Panel for Personal Blog (future)

*Source: `.cursor/plans/plan_admin_panel_blog.md`*
...
## Plan: Write Blog Posts with Gemini (EN/TR, UI-Managed Prompts)

*Source: `docs/plans/plan_gemini_blog_generation.md`*
...
- [ ] 6. Dependency and env — Add `@google/genai` to package.json; update `.env.example` with `GEMINI_API_KEY=` comment.
- [ ] 7. Generation service — `lib/gemini-generate.ts`: `generateBlogDraft()`; read prompts, substitute `{{topic}}`/`{{context}}`, call Gemini, parse with Zod; set date server-side; handle missing API key.
- [ ] 8. Structured output — `blogDraftSchema` in `lib/schemas/blog.ts`; strip markdown code fences; validate Gemini response.
- [ ] 9. Server action for generation — `generateBlogDraftAction(input)` in `lib/actions/gemini.ts`; validate input with Zod; call `generateBlogDraft`.

### Phase 4: UI integration (new post and edit)

- [ ] 10. New post page (BlogPostForm) — "Generate with Gemini": locale (EN/TR), topic text area, Generate button; fill form on success; loading state and error message.
- [ ] 11. Edit page (BlogGroupForm) — "Generate with Gemini" per panel (EN/TR) and "Generate both"; topic field; fill enForm/trForm on success; loading state.
- [ ] 12. Locale behavior — EN only, TR only, or both; no requirement for both locales; existing add/remove locale unchanged.

### Phase 5: Error handling and validation

- [ ] 13. Error handling — Timeout (e.g. 30s), rate limit, parse error messages; admin error boundary if not already present.
- [ ] 14. Validation — Zod for all inputs; topic max 2000 chars; prompt max 10k chars per prompt.

### Phase 6: Documentation and defaults

- [ ] 15. Default prompts — Ship default `content/config/gemini-prompts.json` with EN/TR instructions and `{{topic}}` placeholder.
- [ ] 16. README / .env.example — `.env.example` documents `GEMINI_API_KEY`; README/TODO mention Blog prompts and Generate with Gemini if needed.

## Completed

### Admin Error Handling (Phase 6)

- [x] 9. Admin error boundary and validation — `app/admin/error.tsx` (mirror root, English, Back to dashboard); `lib/api-responses.ts` (badRequestFromZod, serverErrorResponse). Plan completed; plan file removed.
