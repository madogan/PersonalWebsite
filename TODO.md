...
### Plan: Write Blog Posts with Gemini (EN/TR, UI-Managed Prompts) (Tasks 1–16)

- [x] 1–3. Config and prompt storage: `content/config/gemini-prompts.json`; `lib/gemini-prompts.ts`; `lib/schemas/gemini-prompts.ts`; `lib/actions/gemini.ts` (updateGeminiPromptsAction, getDefaultGeminiPromptsAction).
- [x] 4–5. Admin page `/admin/blog-prompts`; nav "Blog prompts"; validation and UX.
- [x] 6–9. `@google/genai`; `.env.example`; `lib/gemini-generate.ts`; `blogDraftSchema`; `generateBlogDraftAction`.
- [x] 10–12. "Generate with Gemini" on new post (BlogPostForm) and edit (BlogGroupForm: EN/TR/both); locale behavior unchanged.
- [x] 13–16. Error handling (timeout, rate limit, parse); validation (topic/prompt length); default prompts; docs.
